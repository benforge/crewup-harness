...
const lessonIndex = await collectLessonIndex();
...
await writeFile(path.join(knowledgeDir, "lesson-index.json"), `${JSON.stringify(lessonIndex, null, 2)}\n`, "utf8");
...
await writeFile(path.join(knowledgeDir, "lesson-index.md"), renderLessonIndex(lessonIndex), "utf8");
...
console.log(`- ${rel(path.join(knowledgeDir, "lesson-index.json"))}`);
console.log(`- ${rel(path.join(knowledgeDir, "decision-index.md"))}`);
console.log(`- ${rel(path.join(knowledgeDir, "lesson-index.md"))}`);
...
async function collectLessonIndex() {
  const rootDir = path.join(knowledgeDir, "lessons");
  const statuses = ["active", "candidates", "archived"];
  for (const status of statuses) {
    await mkdir(path.join(rootDir, status), { recursive: true });
    const marker = path.join(rootDir, status, ".gitkeep");
    if (!existsSync(marker)) await writeFile(marker, "", "utf8");
  }

  const lessons = [];
  for (const statusDir of statuses) {
    const dir = path.join(rootDir, statusDir);
    ...
    for (const entry of entries.filter((item) => item.isFile() && item.name.endsWith(".md"))) {
      ...
      if (!meta.id) continue;
      lessons.push(limitLessonIndexEntry({
        id: meta.id,
        status: meta.status ?? (statusDir === "candidates" ? "candidate" : statusDir.replace(/s$/, "")),
        domain: meta.domain ?? "general",
        confidence: Number(meta.confidence ?? 0),
        scope: asList(meta.scope).length ? asList(meta.scope) : ["general"],
        trigger: cleanListItem(meta.trigger),
        action: cleanListItem(meta.action),
        sourceRun: meta.sourceRun ?? null,
  ...
  lessons.sort((left, right) => {
    const statusRank = { active: 0, candidate: 1, archived: 2 };
  ...
  return {
    ...
    maxEntryChars: 700,
    counts: {
      active: lessons.filter((item) => item.status === "active").length,
      candidate: lessons.filter((item) => item.status === "candidate").length,
      archived: lessons.filter((item) => item.status === "archived").length
    },
    lessons
  };
}
...
function renderLessonIndex(index) {
  const lines = [
    "# Harness Lesson Index",
    "",
    "> Generated by `npm run harness:knowledge`. Lessons are memory hints, not routing authority.",
    "",
    `- generatedAt: ${index.generatedAt}`,
    `- active: ${index.counts.active}`,
    `- candidate: ${index.counts.candidate}`,
    `- archived: ${index.counts.archived}`,
    "",
    "## Active Lessons",
    "",
    "| id | domain | confidence | scope | hint |",
    "| --- | --- | ---: | --- | --- |"
  ];

  const active = index.lessons.filter((item) => item.status === "active");
  if (active.length) {
    lines.push(...active.map(renderLessonRow));
  } else {
    lines.push("| none | - | 0 | - | - |");
  }

  const candidates = index.lessons.filter((item) => item.status === "candidate").slice(0, 25);
  lines.push(
    "",
    "## Candidate Lessons",
    "",
  ...
  if (candidates.length) {
    lines.push(...candidates.map(renderLessonRow));
  } else {
    lines.push("| none | - | 0 | - | - |");
  }
  ...
  lines.push(
    ...
    "## Usage",
    "",
    "- `active` lessons may appear in short memory hints.",
    "- `candidate` lessons are review material and are not treated as active rules.",
    "- `archived` lessons are retained for audit only.",
    ""
  );
...
function renderLessonRow(item) {
  return `| \`${item.id}\` | ${item.domain} | ${item.confidence} | ${item.scope.join(", ")} | ${cell([item.trigger, item.action].filter(Boolean).join(" -> "))} |`;
}
...
function renderRefreshReport(index, board) {
  return [
    ...
    "- `.harness/knowledge/lesson-index.json`",
    "- `.harness/knowledge/decision-index.md`",
    "- `.harness/knowledge/lesson-index.md`",
...
function limitLessonIndexEntry(entry) {
  return {
    ...entry,
    trigger: limitText(entry.trigger ?? "", 160),
    action: limitText(entry.action ?? "", 180),
    scope: compactList(entry.scope, 8, 80)
  };
}
