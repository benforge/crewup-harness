...
启动 native subagent 前必须检查 plan 中的 `requires_completed_agents`。前置 agent 没有完成并通过 `native-state mark-result` 捕获结果时，不要提前启动后续 agent。规划类 run 的正常顺序是 `requirements-plan` -> `requirements` -> `architect` -> `reviewer`，除非本次 plan 明确省略其中某个角色。

正式 artifact 必须由 owner agent 写入。主 agent 可以捕获 result、检查 gate、要求修复和汇总状态，但不能把子 agent 返回的正文复制成 `requirement-plan.md`、`requirement.md`、`architecture.md`、`implementation-plan.md` 等 owner artifact。
