...
这份文档面向维护者，用来说明 CrewUp 的公开入口、核心流水线、兼容工具和维护脚本边界。普通使用者不需要记住所有 `.harness/scripts` 文件，只需要理解：
