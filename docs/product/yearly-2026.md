# 2026 年目标

## 产品方向

- 先完成个人博客的最小可用版本。
- 再扩展后台管理、评论、搜索、媒体库、SEO、RSS、部署流水线等能力。

## 年度原则

- 每个需求都必须进入 harness 流程。
- 每个开发 run 都要留下需求、方案、变更、测试和发布摘要。
- 不追求一次性全自动，先追求可追踪、可复盘、可持续迭代。

## 年度运行索引

<!-- harness:run-sync:start 2026-05-15-001-blog-mvp -->
- 2026-05-15 [001 个人博客 MVP](runs/2026-05-15-001-blog-mvp.md)
  - run: `2026-05-15-001-blog-mvp`
<!-- harness:run-sync:end 2026-05-15-001-blog-mvp -->

<!-- harness:run-sync:start 2026-05-18-002-complete-blog-system -->
- 2026-05-18 [002 完善博客系统 MVP](runs/2026-05-18-002-complete-blog-system.md)
  - run: `2026-05-18-002-complete-blog-system`
<!-- harness:run-sync:end 2026-05-18-002-complete-blog-system -->

<!-- harness:run-sync:start 2026-05-19-004-c-end-visual-polish -->
- 2026-05-19 [004 C 端视觉与内容层级专项优化](runs/2026-05-19-004-c-end-visual-polish.md)
  - run: `2026-05-19-004-c-end-visual-polish`
<!-- harness:run-sync:end 2026-05-19-004-c-end-visual-polish -->

<!-- harness:run-sync:start 2026-05-20-005-it-engineer-blog-experience-redesign -->
- 2026-05-20 [005 IT 工程师博主博客体验专业化重设计](runs/2026-05-20-005-it-engineer-blog-experience-redesign.md)
  - run: `2026-05-20-005-it-engineer-blog-experience-redesign`
<!-- harness:run-sync:end 2026-05-20-005-it-engineer-blog-experience-redesign -->

<!-- harness:run-sync:start 2026-05-22-008-现在迭代一个功能-增加阅读量这个统计 -->
- 2026-05-22 [现在迭代一个功能，增加阅读量这个统计](runs/2026-05-22-008-现在迭代一个功能-增加阅读量这个统计.md)
  - run: `2026-05-22-008-现在迭代一个功能-增加阅读量这个统计`
<!-- harness:run-sync:end 2026-05-22-008-现在迭代一个功能-增加阅读量这个统计 -->

<!-- harness:run-sync:start 2026-05-26-012-迭代一个需求-相册这个照片瀑布流希望是沾满屏幕的-宽度这块-把网站这块英文都修改 -->
- 2026-05-26 [012 迭代一个需求 相册这个照片瀑布流希望是沾满屏幕的 宽度这块 把网站这块英文都修改](runs/2026-05-26-012-迭代一个需求-相册这个照片瀑布流希望是沾满屏幕的-宽度这块-把网站这块英文都修改.md)
  - run: `2026-05-26-012-迭代一个需求-相册这个照片瀑布流希望是沾满屏幕的-宽度这块-把网站这块英文都修改`
<!-- harness:run-sync:end 2026-05-26-012-迭代一个需求-相册这个照片瀑布流希望是沾满屏幕的-宽度这块-把网站这块英文都修改 -->
