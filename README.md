...
CrewUp 是一套面向真实工程仓库的 AI 协作工作流框架。它把需求入口、上下文整理、角色分工、子 agent 协作、质量门禁、交付报告和归档提交串成一条可追踪的闭环，让 AI 迭代不再停留在一次性的对话里。

它不绑定具体技术栈，也不要求项目必须采用 `apps/`、`packages/` 或 monorepo 结构。无论你的项目是 Web、后端、脚本工具、桌面应用，还是混合工程，CrewUp 都只负责提供通用协作协议；真实项目结构由 `crewup init` 在目标仓库内识别并生成适配层。
...
`crewup` 是 npm 包名和完整 CLI 名称；`harness` 是内部工作流概念。
...
1. `install` 先把通用工作流装进项目
2. `inspect` 读取真实目录、语言和脚本情况
3. `init` 生成项目适配层
4. `check` 确认配置和核心文件都能闭环
...
如果某个 skill 已经安装，CrewUp 会优先读取已安装路径；如果只是候选项，就只按配置和文档引导，不假设它已经可用。
